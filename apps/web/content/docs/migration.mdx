...
| `BuzzFormProvider` | `<FormProvider registries={{ fields: ... }}>` |
...
5. **Provider** — `BuzzFormProvider` → `<FormProvider registries={{ fields: ... }}>`
6. **Registries** — `registry`, `customValidators`, and `optionResolvers` unified into a single `registries` object
7. **Validation** — Function-based → `ValidationConfig` with `checks` arrays
8. **Conditions** — Function-based → `Condition` AST
9. **Terminology** — `DynamicValue` → `Expr<T>`, `VisibilityCondition` → `Condition`
10. **Array props** — `minRows`/`maxRows` → `minItems`/`maxItems`
11. **Dependencies** — `dependencies` array removed, automatic via `$data`
