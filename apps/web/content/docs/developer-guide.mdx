...
<Form schema={schema} registries={{ fields: registry }} onSubmit={handleSubmit}>
  <FormContent>
    <FormFields />
  </FormContent>
</Form>;
...
```ts
import type { FormSchema, InferType } from "@buildnbuzz/form-react";
...
**Problem:** Form can't find the configuration or registry.

**Solution:** Either wrap with `FormProvider` or pass `registries.fields` explicitly.
...
// Option B: Pass registries to each form
<Form schema={schema} registries={{ fields: registry }} onSubmit={handleSubmit} />
```
