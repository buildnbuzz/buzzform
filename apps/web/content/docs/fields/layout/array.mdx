...
|--------|------|---------|-------------|
| `variant` | `"default" | "minimal"` | `"default"` | Visual style for the array container |
...
| `rowLabel` | `unknown` | `"Item #N"` | Row header label — use `{ $data: "fieldName" }` (or `{ $data: "/value" }` for flat arrays) |
...
Use `mode: "flat"` for arrays of single values. Omit the item `name` to map
the item value directly into the array.
...
If you provide a `name`, each item is wrapped in an object:
...
For flat arrays, use `{ $data: "/value" }` to reference the current item:
