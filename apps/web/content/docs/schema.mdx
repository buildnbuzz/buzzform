...
### Hiding Required Asterisks

By default, required fields display a red required asterisk (`*`) next to their label. You can hide the asterisk on a per-field basis by setting the `ui.asterisk` option to `false`:
